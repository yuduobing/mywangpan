...
import com.micro.disk.bean.MergeRequest2;
...
public class FileUploadController {
	...
	@Reference(check = false,timeout=120000)
	private ShareService shareSercvice;
	...
	@ApiOperation(value="临时任务合并返回分享",notes="切块合并")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "bean", value = "参数实体",dataType = "String",paramType="query",required=true),
			@ApiImplicitParam(name = "token", value = "token",dataType = "String",paramType="query",required=true)
	})
	@PostMapping("/mergeChunk2")
	public Result mergeChunk2(MergeFileBean bean, HttpServletRequest request) {
		try {
			SessionUserBean user = UserInfoUtils.getBean(request);
			bean.setUserid(user.getId());
			bean.setUsername(user.getNickname());
			Map<String,String> o = (Map)fileService.mergeChunk2(bean);
			//在serverAPi2定义了不能映入provider应为他映入了数据库
			MergeRequest2 s= JSON.parseObject(JSON.toJSONString(o), MergeRequest2.class);

			ArrayList<String> strings = new ArrayList<>();
			strings.add(s.getDiskfileid());
//			String username,Integer sharetype,Integer effect,Integer type);
			ShareSecretResult shareSecretResult = shareSercvice.shareSecret(strings, s.getFilename(), s.getUserid(), s.getUsername(), 1, 1, 0);
			return ResultUtils.success("合并成功并分享", shareSecretResult);
		} catch (Exception e) {
			return ResultUtils.error(e.getMessage());
		}
	}
